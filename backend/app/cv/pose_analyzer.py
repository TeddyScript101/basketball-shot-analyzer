...
class PoseAnalyzer:
    ...
    Detects key biomechanical metrics from a trimmed shot video clip:
    - Release angle (ideal 45-55 degrees)
    - Elbow angle at release (ideal 85-100 degrees)
    - Knee bend at setup (ideal 90-120 degrees)
    - Shoulder alignment deviation
    - Jump height estimate (normalized)
    - Release consistency score
    - Overall composite shooting score 0-100
    """

    METRIC_RANGES = {
        "release_angle": (45.0, 55.0),
        "elbow_angle": (85.0, 100.0),
        "knee_angle": (90.0, 120.0),
        "shoulder_alignment": (0.0, 3.0),
        "jump_height": (0.03, 0.12),
    }
    ...
    def _detect_shooting_arm(self, frames_data: list[FrameData]) -> str:
        """Determine dominant shooting arm by comparing peak wrist heights."""
        right_peaks, left_peaks = [], []
        for fd in frames_data:
            lm = fd.landmarks
            right_peaks.append(1.0 - lm[PoseLandmark.RIGHT_WRIST].y)
            left_peaks.append(1.0 - lm[PoseLandmark.LEFT_WRIST].y)
        return "right" if max(right_peaks) >= max(left_peaks) else "left"
    ...
    def _detect_release_frame(self, frames_data: list[FrameData], arm: dict) -> int:
        """
        Release frame proxy: frame with highest weighted combination of
        wrist height and arm extension (lower elbow angle = more extended).
        """
        scores = []
        for fd in frames_data:
            lm = fd.landmarks
            wrist_height = 1.0 - lm[arm["wrist"]].y

            shoulder = lm_xy(lm[arm["shoulder"]])
            elbow = lm_xy(lm[arm["elbow"]])
            wrist = lm_xy(lm[arm["wrist"]])
            elbow_angle = calculate_angle(shoulder, elbow, wrist)
            extension = 1.0 - (elbow_angle / 180.0)

            scores.append(wrist_height * 0.65 + extension * 0.35)
    ...
    def _compute_metrics(self, frames_data: list[FrameData], fps: float) -> ShootingMetrics:
        ...
        # --- Shoulder alignment: deviation of shoulder level at release ---
        sh_main_y = lm[arm["shoulder"]].y
        sh_opp_y = lm[arm["opp_shoulder"]].y
        shoulder_alignment = float(abs(sh_main_y - sh_opp_y) * 100.0)

        # --- Shot duration: frames from wrist motion onset to release ---
        ...
        # --- Jump height: normalized hip Y displacement in pre-release window ---
        ...
        # --- Release consistency: smoothness of wrist trajectory ---
